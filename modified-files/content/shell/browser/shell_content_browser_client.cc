...
blink::UserAgentMetadata GetShellUserAgentMetadata() {
...
#if BUILDFLAG(IS_IOS)
  // Present as mobile Chrome (matching GetUserAgent) so Client Hints agree
  // with the UA string.
  metadata.brand_version_list.emplace_back("Chromium", "124");
  metadata.brand_version_list.emplace_back("Google Chrome", "124");
  metadata.brand_version_list.emplace_back("Not-A.Brand", "99");
  metadata.brand_full_version_list.emplace_back("Chromium", "124.0.6367.207");
  ...
  metadata.full_version = "124.0.6367.207";
  metadata.platform = "Android";
  metadata.platform_version = "14.0.0";
...
// Set by the iOS Appearance UI (shell_platform_delegate_ios.mm).
// 0 = default (light), 1 = light, 2 = dark. Drives web prefers-color-scheme.
...
void ShellContentBrowserClient::OverrideWebPreferences(
  ...
  // content_shell hardcodes prefers-color-scheme to light; honor the user's
  // Appearance choice instead. 2 == dark.
...
#if BUILDFLAG(IS_IOS)
  // Request Desktop Site: ignore the page's mobile <meta viewport> and lay
  // out at Blink's legacy ~980 CSS px desktop width, shrunk to fit. Mobile
  // mode leaves the platform defaults.
...
#if BUILDFLAG(IS_IOS)
  // content_shell's default UA reports "Chrome/<ver>" on an iOS platform, an
  // impossible combination that trips sites' unsupported-browser checks.
  // Present as a current mobile Chrome instead; the engine really is
  // Blink/V8. Request Desktop Site switches to a desktop UA.
  return "Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 (KHTML, "
...
void ShellContentBrowserClient::ConfigureNetworkContextParamsForShell(
  ...
  // content_shell defaults to an unbounded in-memory HTTP cache, which
  // OOM-crashes the renderer on heavy sites. Cap the cache and put it on
  // disk so resources spill to storage instead of RAM.
  ...
  if (!context->IsOffTheRecord()) {
    ...
    // Persist cookies so logins survive app close/reopen. Stock
    // content_shell leaves cookie_database_name empty, which forces an
    // in-memory cookie store. Set data_directory + cookie_database_name for
    // the SQLite-backed store, and persist session cookies too.
...
#endif
    // Store cookies unencrypted: the encrypted-cookie path depends on
    // OSCrypt, which content_shell doesn't initialize on iOS. The data lives
    // in the app's own sandboxed container.
    context_params->enable_encrypted_cookies = false;
  }

#if BUILDFLAG(IS_IOS)
  // Route traffic through the user's proxy (Settings -> Tor / Proxy, saved
  // as NSUserDefaults "BlinkProxy"). Set on the network context directly; the
  // command-line switch alone isn't honored. SOCKS5 does remote DNS, so
  // .onion resolves through Tor.
