...
// Boot tracer (defined in shell_application_ios.mm). Declared here so the
// earliest framework code can mark progress, distinguishing a crash during
// image load / static initialization from one inside startup.
...
// Capture a crash backtrace into blink_boot.log; the device's ReportCrash is
// disabled. Async-signal safe: open()/write()/backtrace_symbols_fd() only, no
// malloc/Foundation.
...
// Arms the crash handler via sigaction on an alternate stack (survives stack
// overflow). Exposed extern "C" so it can be re-armed after Chromium installs
// its own signal handlers during startup. Safe to call repeatedly.
