...
class CONTENT_EXPORT RenderWidgetHostViewIOS
 ...
 public:
  ...
  // The focused editable's caret bottom as a ratio [0,1] of the viewport
  // height, from Blink's selection/caret metrics. Returns -1 if unavailable.
  float GetFocusedInputBottomRatio();

  // Asks the renderer, via isolated-world JS on the primary main frame, for
  // the focused editable's bottom edge as a fraction of the visual viewport
  // height, widened to its prompt-box ancestor. Works even when the
  // browser-side TextInputManager has no active widget. Async; callback runs
  // on the UI thread with -1 when unavailable.
