...
ImageTransportSurfaceOverlayMacEGL::ImageTransportSurfaceOverlayMacEGL(
  ...
  // Instead of wrapping the GPU's root CALayer in a BELayerHierarchy and
  // shipping it to the browser over XPC (BrowserEngineKit, iOS 17.4+ only),
  // attach it directly to the on-screen view. Single-process, so the GPU
  // thread and the browser UI share an address space: look up the
  // CALayerFrameSinkProvider by surface handle and add the rendered layer as
  // a sublayer.
