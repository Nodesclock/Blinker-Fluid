...
bool PartitionAddressSpace::IsIOSTestProcess() {
  ...
  // This process gets only ~4 GB of reservable address space (the
  // extended-virtual-addressing entitlement is not honored), the same
  // constraint described above for test processes. Force the small pool size
  // so PartitionAlloc's pool reservation fits; otherwise PartitionRoot init
  // traps in HandlePoolAllocFailure at the first malloc.
  return true;
