...
#if BUILDFLAG(IS_IOS)
  // Single-process iOS has no supported camera/mic device path. Return a
  // safe denial instead of spinning up AVFoundation device enumeration.
...
- (bool)isPortraitEffectSupported {
  ...
  // -portraitEffectSupported is iOS 15+; report unsupported below that.
...
- (bool)isPortraitEffectActive {
  ...
  // -portraitEffectActive is iOS 15+; report inactive below that.
  if (__builtin_available(iOS 15, macOS 12, *)) {
