...
// Write address for zeroing swept memory: the RW mirror for a code-range address
// on the iOS mirror build (the source is RX), the identity otherwise.
V8_INLINE void* BlinkCodeChunkWriteDst(Address addr) {
#if V8_HEAP_USE_IOS_MPROTECT_JIT_WRITE_PROTECT
  return reinterpret_cast<void*>(
      IOSCodeRangeWriteProtect::MirrorWriteAddress(addr));
#else
...
void Sweeper::ZeroOrDiscardUnusedMemory(NormalPage* page, Address addr,
  ...
  if (kDiscardEmptyPages && discard_area) {
    ...
    if (v8_flags.zero_unused_memory) {
      ...
      // help with OS page compression. On iOS a code page is RX (the scope above
      // is a no-op under the mirror), so route the zeroing through the RW mirror;
      // BlinkCodeChunkWriteDst is the identity for non-code memory.
      memset(BlinkCodeChunkWriteDst(unused_start), 0,
             discard_area->begin() - unused_start);
      memset(BlinkCodeChunkWriteDst(discard_area->end()), 0,
