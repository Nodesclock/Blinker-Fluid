...
#if defined(V8_TARGET_OS_IOS)
// Blinker Fluid iOS jailbreak port: mirror a one-line marker into the boot log
// (same file BlinkBootLog/BlinkJitMark use) so we can see, off-device, exactly
// which CodeRange reservation size the retry loop below actually got. Plain
// stdio; runs during Isolate::Initialize on the renderer thread.
...
// Diagnostic: reserve `bytes` with the SAME minimal flags the standalone VA
// probe used (PROT_NONE, MAP_ANON|MAP_PRIVATE, fd=-1, NO MAP_NORESERVE, NO VM
// tag) — i.e. what V8's VirtualMemoryCage does NOT use. If this succeeds where
// the cage reservation just failed at the same size, the wall is V8's mmap
// flags (NORESERVE / VM tag / alignment), not genuine VA exhaustion. Logs the
// outcome + errno and immediately unmaps. Returns true on success.
...
bool CodeRange::InitReservation(v8::PageAllocator* page_allocator,
...
#if defined(V8_TARGET_OS_IOS)
  // Blinker Fluid iOS jailbreak port: this single-process app gets only ~4 GB
  // of reservable virtual address space (no extended-virtual-addressing), and
  // by the time V8 initializes, PartitionAlloc's pools, the V8/Blink heaps and
  // the ~260 MB framework image have fragmented it. A single 128 MB
  // (kMaximalCodeRangeSize) PROT_NONE reservation therefore succeeds only
  // INTERMITTENTLY, and the stock iOS path makes exactly one attempt and then
  // calls FatalProcessOutOfMemory — which is precisely the coin-flip
  // "renderer OOMs at InitCodeRangeOnce → black screen" that made JIT
  // unshippable on this device (confirmed by on-device crash reports). Instead,
  // try the requested size and, on failure, shrink toward kMinimumCodeRangeSize
  // and retry: V8 takes the largest code range that currently fits (a browser
  // runs fine on 64 MB — it just flushes JIT code sooner), and a tight-VA
  // moment degrades gracefully instead of aborting the renderer. The write+exec
  // of the committed code pages already works via plain RWX (MAP_JIT is dropped
  // on iOS in platform-posix.cc and is EINVAL on this jailbreak anyway); the
  // only remaining JIT wall was this reservation, so making it robust is what
  // turns JIT from a black screen into a working, faster engine.
  ...
  for (;;) {
    ...
    // Diagnose: does a plain PROT_NONE mmap (VA-probe flags) fit this size?
  ...
  if (!code_range_reserved) {
    CodeRangeBootLog("all attempts failed, staying jitless-equivalent",
                     requested);
    // Diagnose the real ceiling: find the largest plain PROT_NONE mmap that
    // fits right now. If even a few MB fails, the app's VA is genuinely
    // exhausted/fragmented (fix = free VA); if 32-48MB fits but the cage
    // didn't, the wall is V8's reservation flags/alignment (fix = reserve with
    // plain flags). Runs only on total failure, so it never costs a working
    // boot.
