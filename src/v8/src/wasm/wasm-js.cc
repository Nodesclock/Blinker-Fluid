...
void WasmJs::Install(Isolate* isolate) {
  ...
  // iOS port: --jitless implies --wasm-jitless (flag implication), which is
  // meant to route Wasm to the DrumBrake interpreter. But DrumBrake is only
  // compiled in when v8_enable_drumbrake=true (it needs pointer compression,
  // which this device's ~4GB virtual-address budget can't afford). Without the
  // interpreter, exposing WebAssembly and then instantiating a module falls
  // through to the JIT compile path with no executable memory and crashes the
  // renderer INSTANTLY (ChatGPT, Home Depot product pages, many SPAs). So only
  // honor wasm_jitless for exposure when the interpreter actually exists; with
  // it absent, WebAssembly stays undefined and sites feature-detect & fall back.
