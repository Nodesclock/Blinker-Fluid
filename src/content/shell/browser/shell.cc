...
#if BUILDFLAG(IS_IOS)
// Memory limits for the single-process iOS runtime.
constexpr bool kEnableIOSGlobalLowMemoryGuard = true;
constexpr uint64_t kGlobalSoftPurgeBytes =
    blinker_memory::kModerateFootprint;
constexpr uint64_t kGlobalCriticalPurgeBytes =
    blinker_memory::kCriticalFootprint;
constexpr uint64_t kGlobalBlockNewContentsBytes =
    blinker_memory::kBlockNewContentsFootprint;
...
void UpdateKeyboardChatFallback(const GURL& url) {
  ...
  g_keyboard_chat_fallback_ratio = chat ? 0.92f : 0.0f;
  char buf[160];
...
bool ShouldBlockNewWebContents(const char* path, const GURL& url) {
  ...
  if (stats.phys_footprint < kGlobalBlockNewContentsBytes) {
    char allowed[256];
