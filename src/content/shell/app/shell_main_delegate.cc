...
bool IsIOSRuntimeJITAuthorized() {
  ...
  if (!bundle_id ||
      CFStringCompare(bundle_id,
                      CFSTR("com.nodesclock.blinkerfluid.jittest3"),
                      0) != kCFCompareEqualTo) {
    AppendBlinkBootLog(
        "JIT_AUTH: stable bundle selected; using jitless engine\n");
    return false;
  }
  // TrollStore's "Open with JIT" sets CS_DEBUGGED on the running process.
  // Executable-memory probes are not sufficient on iOS 15: mappings may be
  // created successfully while executing generated V8 code still raises
  // SIGBUS.
  constexpr uint32_t kCSDebugged = 0x10000000;
  uint32_t status = 0;
  if (csops(getpid(), CS_OPS_STATUS, &status, sizeof(status)) != 0) {
    AppendBlinkBootLog(
        "JIT_AUTH: csops failed; using safe jitless fallback\n");
    return false;
  }
...
std::optional<int> ShellMainDelegate::BasicStartupComplete() {
  ...
  if (!command_line.HasSwitch("js-flags")) {
    ...
    if (runtime_jit_authorized) {
      command_line.AppendSwitchASCII(
          "js-flags",
          "--no-sparkplug --regexp-interpret-all --wasm-jitless "
          "--disable-optimizing-compilers "
          "--max-old-space-size=256 "
          "--max-semi-space-size=8");
      AppendBlinkBootLog(
          "JIT_AUTH: safe hybrid mode; native JS/RegExp/Wasm codegen disabled\n");
    } else {
      command_line.AppendSwitchASCII(
          "js-flags",
          "--jitless --wasm-jitless "
          "--max-old-space-size=256 --max-semi-space-size=8");
      AppendBlinkBootLog(
