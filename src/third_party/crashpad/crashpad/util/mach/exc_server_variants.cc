...
#if BUILDFLAG(IS_IOS)
// iOS-14 back-deploy: task_identity_token_get_task_port() (used in the protected
// exception path below) is an iOS 16+ Mach routine. <mach/task.h> declares it as
// a MIG-generated prototype with NO availability annotation, so clang emits a
// STRONG import even when building at a lower deployment target. iOS 14's
// libSystem doesn't export the symbol, and arm64 chained fixups bind every
// import eagerly at launch — so the process aborts before main() with "Symbol
// not found: _task_identity_token_get_task_port". Re-declare it weak_import so
// dyld binds it to NULL instead of aborting. The only caller is the protected-
// exception message, which a pre-16 kernel never delivers, and it's null-guarded
// at the call site for safety.
...
#if BUILDFLAG(IS_IOS)
      // Weak-imported for iOS 14 back-deploy (see top of file); NULL on iOS < 16.
