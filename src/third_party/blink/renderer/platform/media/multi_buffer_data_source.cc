...
#if BUILDFLAG(IS_IOS)
  // Blinker Fluid iOS OOM guard: on a memory-constrained, single-process,
  // jitless device the pinned media regions (which cannot be evicted by the
  // GlobalLRU or by memory pressure — MultiBuffer ignores pressure) are a
  // direct path to a jetsam SIGKILL. A single large <video>/<audio> could
  // otherwise pin hundreds of MB (the fully-cached / small-file branch above
  // pins length*2). Clamp the pinned and advisory buffer sizes to hard iOS
  // ceilings; the cost is occasional re-fetching, never incorrect playback.
  constexpr int64_t kIOSMaxBuffer = 24 << 20;    // 24 Mb advisory cache
  constexpr int64_t kIOSMaxPinForward = 24 << 20;   // 24 Mb pinned ahead
  constexpr int64_t kIOSMaxPinBackward = 8 << 20;   // 8 Mb pinned behind
  buffer_size = std::min(buffer_size, kIOSMaxBuffer);
  pin_forward = std::min(pin_forward, kIOSMaxPinForward);
  pin_backward = std::min(pin_backward, kIOSMaxPinBackward);
#endif
