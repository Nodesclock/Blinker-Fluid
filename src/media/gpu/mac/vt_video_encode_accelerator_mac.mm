...
CreateCompressionSession(VideoCodec codec,
  ...
  if (@available(macOS LOW_LATENCY_AND_SVC_AVAILABLE_VER, iOS 15.0, *)) {
    ...
    if (required_encoder_type != EncoderType::kSoftware && require_low_delay &&
      ...
      // iOS-14 back-deploy: this key is iOS 14.5+ (weak -> NULL on iOS 14.0-14.4,
      // and a nil NSDictionary key would throw). Skip low-latency rate control
      // on those releases.
