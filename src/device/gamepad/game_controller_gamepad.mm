...
#define BUTTON(i, b)                      \
  ...
  if (GCXboxGamepad* xbox_gamepad =
    ...
    // iOS-14 back-deploy: -buttonShare is iOS 15+; the selector is absent on
    // iOS 14's GCXboxGamepad (a live object, so messaging it would throw), so
    // guard the access. (The GCDualSenseGamepad/GCDualShockGamepad branches
    // below are class lookups that resolve to nil on iOS 14 and self-disable.)
