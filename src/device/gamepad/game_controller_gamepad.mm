...
#define BUTTON(i, b)                      \
  ...
  if (GCXboxGamepad* xbox_gamepad =
    ...
    // buttonShare is unavailable before iOS 15.
    ...
    if (@available(iOS 14.5, *)) {
      if (GCDualSenseGamepad* dualsense_gamepad =
    ...
    if (!handled) {
      if (GCDualShockGamepad* dualshock_gamepad =
...
void GameControllerGamepad::InitializeStaticData(Gamepad& pad) {
  ...
  bool supports_touch_events = false;
  if (@available(iOS 14.5, *)) {
    supports_touch_events =
        [controller_ isKindOfClass:[GCDualSenseGamepad class]];
  }
